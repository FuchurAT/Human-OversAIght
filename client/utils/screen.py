...
class Screen(QMainWindow):
    def __init__(self, screen_id: int = 0, name: str = "ScreenManager", show_focus: bool = False):
        """
        ...
        for idx, input in enumerate(self.inputs):
            ...
            if isinstance(input, list):
                ...
                for message in input:
                    ...
                    # Keep track of messages (max 20)
                    self.messages = self.messages[-20:] if len(self.messages) > 20 else self.messages
                    ...
                    for msg_idx, msg in enumerate(self.messages):
                        ...
                        # Add label to layout
                        text_layout.addWidget(label, msg_idx, 0)
                        
                        # Start typing animation for the last message
                        if msg_idx == len(self.messages) - 1:
                            self._start_typing_animation(label, msg.replace('"', ''))
                        else:
                            label.setText(msg.replace('"', ''))
                    
                    # Add spacer to push messages to the top
                    text_layout.setRowStretch(len(self.messages), 1)
                ...
                if pixmap.isNull():
                    ...
                    # Scale image maintaining aspect ratio and respecting max width
